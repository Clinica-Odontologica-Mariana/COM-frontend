    ...
    <main class="bg-[#FAFAF9] text-[#6B5B52]">
      ...
      <section id="sobre" class="mx-auto max-w-7xl px-6 py-16 lg:px-12 lg:py-20">
        <div class="grid items-center gap-12 lg:grid-cols-[1.05fr_0.95fr]">
          <div>
            ...
            <h1 class="max-w-3xl font-serif text-4xl leading-[1.05] text-[#8B574B] sm:text-5xl lg:text-6xl">
              Dra. Mariana, transformando sorrisos onde você estiver.
            </h1>
            <p class="mt-6 max-w-2xl text-sm leading-7 text-[#6D625E] sm:text-base">
              Minha jornada começou na Universidade de Brasília (UnB) quando me encantei pela odontologia.
              Desde cedo, sou movida pelo desejo de ajudar as pessoas sendo realçando sua beleza e confiança
              ou restaurando sua saúde. Por isso me encantei pela Odontologia Estética e hoje aprofundo meus
              conhecimentos na Especialização de Dentística na USP Ribeirão Preto.
            </p>
          ...
          <div class="mt-12 grid gap-6 md:grid-cols-3">
            @for (item of timeline; track item.year) {
              <div class="relative rounded-2xl bg-white px-6 py-8 shadow-sm ring-1 ring-[#F0E8E3]">
                <div class="mx-auto mb-4 grid h-10 w-10 place-items-center rounded-full bg-[#8B574B] text-white">
                  ✦
                </div>
                <p class="text-xs font-semibold uppercase tracking-[0.2em] text-[#B49B91]">{{ item.year }}</p>
      ...
      <section id="atendimento" class="mx-auto max-w-7xl px-6 py-16 lg:px-12">
        ...
        <div class="mt-10 grid gap-4 lg:grid-cols-[1.15fr_0.85fr]">
          <article class="tilt-card relative min-h-[340px] overflow-hidden rounded-[28px] bg-[#8A5C4D] text-white shadow-[0_18px_45px_rgba(138,92,77,0.22)]">
            <div class="absolute inset-0 bg-[radial-gradient(circle_at_20%_20%,rgba(255,255,255,0.08),transparent_30%),radial-gradient(circle_at_80%_30%,rgba(255,255,255,0.1),transparent_25%),linear-gradient(135deg,#A26A59_0%,#8A5C4D_45%,#6D4338_100%)]"></div>
            <div class="absolute inset-0 opacity-40">
              <div class="absolute left-8 top-10 h-20 w-20 rounded-full border border-white/25"></div>
              <div class="absolute right-10 top-14 h-32 w-32 rounded-full border border-white/15"></div>
              <div class="absolute bottom-10 left-16 h-44 w-44 rounded-full border border-white/10"></div>
            </div>
            <div class="absolute bottom-0 left-0 right-0 p-7">
              <h3 class="font-serif text-3xl">Reabilitação Oral</h3>
              <p class="mt-3 max-w-md text-sm leading-6 text-white/85">
                Restaurando a função mastigatória e a estética através de próteses e implantes com tecnologia guiada.
              </p>
            </div>
          </article>
          ...
          <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-1">
            <article class="rounded-[28px] bg-[#F7D7D1] p-7 shadow-sm">
              ...
              <p class="mt-4 text-sm leading-6 text-[#6F625D]">
                Atendimento especializado para a terceira idade, com protocolos adaptados para pacientes com Alzheimer ou limitações motoras.
              </p>
            </article>
      ...
      <section class="bg-[#F3F3F3] px-6 py-16 lg:px-12">
        <div class="mx-auto max-w-7xl text-center">
          ...
          <p class="mt-2 text-sm text-[#A29087]">Um novo paradigma de cuidado que une a precisão técnica ao calor humano.</p>

          <div class="mt-10 grid gap-4 lg:grid-cols-3">
            @for (item of philosophy; track item.title) {
              <article class="rounded-[22px] bg-white p-8 text-left shadow-sm ring-1 ring-[#EFE8E4]">
                <div class="grid h-12 w-12 place-items-center rounded-full bg-[#F5D7CF] text-[#8B574B]">
                  {{ item.icon }}
                </div>
      ...
      <section class="px-6 py-20 lg:px-12">
        <div class="mx-auto max-w-5xl rounded-[32px] bg-[#A06D5E] px-6 py-16 text-center text-white shadow-[0_18px_60px_rgba(160,109,94,0.25)]">
