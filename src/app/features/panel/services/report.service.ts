...
export class ReportService {
  ...
  async generatePDF(options: ReportOptions): Promise<void> {
    ...
    pdf.text(`Gerado em ${new Date().toLocaleDateString('pt-BR')}`, W - margin, 20, {
      align: 'right',
    });
    ...
    const cards = [
      ...
      },
      {
        ...
        color: [167, 119, 105] as [number, number, number],
    ...
    const barH = barW * (220 / 580);
    pdf.addImage(barImg, 'PNG', margin, y, barW, barH);

    const pieW = contentW * 0.28;
    const pieH = pieW;
    const pieX = margin + contentW * 0.63;
    pdf.addImage(pieImg, 'PNG', pieX, y, pieW, pieH);

    let legendY = y + pieH + 4;
    pdf.setFont('helvetica', 'normal');
    pdf.setFontSize(7);

    dashboardData.revenueByService.forEach((item) => {
      pdf.setFillColor(item.color);
      pdf.rect(pieX, legendY, 3, 3, 'F');

      pdf.setTextColor(80, 80, 80);
      const valStr = item.value.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' });

      let label = item.label;
      if (label.length > 22) label = label.substring(0, 20) + '...';

      pdf.text(`${label} — ${valStr}`, pieX + 5, legendY + 2.5);
      legendY += 4.5;
    });

    y = Math.max(y + barH, legendY) + 8;
    ...
    let cx = margin + 2;
    pdf.text('DATA', cx, y + 4.5);
    cx += cols.date;
    pdf.text('DESCRIÇÃO', cx, y + 4.5);
    cx += cols.desc;
    pdf.text('CATEGORIA', cx, y + 4.5);
    cx += cols.cat;
    pdf.text('STATUS', cx, y + 4.5);
    cx += cols.status;
    ...
    result.items.forEach((item, idx) => {
      ...
      pdf.setFontSize(7.5);
      pdf.setTextColor(100, 100, 100);
      pdf.text(item.status, cx, y + 4.5);
      cx += cols.status;

      const valueStr = item.value.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' });
      pdf.setTextColor(
        ...((item.value > 0 ? [6, 78, 59] : [127, 29, 29]) as [number, number, number]),
      );
      pdf.setFont('helvetica', 'bold');
