...
@Component({
  ...
  template: `
    <div
      class="p-4 sm:p-6 md:p-8 flex flex-col gap-4 sm:gap-5 font-sans text-stone-800 bg-stone-50/50 min-h-screen"
    >
      <header class="flex flex-col sm:flex-row sm:items-start justify-between gap-3 sm:gap-4">
        <div>
          <h1
            class="font-serif text-xl sm:text-2xl md:text-[26px] font-bold text-stone-900 m-0 mb-1"
          >
            Painel Financeiro
          </h1>
        ...
        <div class="flex gap-2">
          <button
            ...
            class="flex items-center justify-center gap-2 px-3 sm:px-4 py-2 sm:py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white rounded-lg text-xs sm:text-sm font-bold tracking-wide whitespace-nowrap transition-colors"
          >
            ＋ Nova Transação
          </button>
          <button
      ...
      >
        <app-panel-card
          label="RECEITA TOTAL"
          [value]="
            (data.stats.monthlyRevenue | currency: 'BRL' : 'symbol' : '1.2-2' : 'pt-BR') ||
            'R$ 0,00'
          "
          [badgeText]="revenueBadge"
          type="revenue"
        />
        <app-panel-card
          label="DESPESAS"
          [value]="
            (data.stats.monthlyExpenses | currency: 'BRL' : 'symbol' : '1.2-2' : 'pt-BR') ||
            'R$ 0,00'
          "
          [badgeText]="expenseBadge"
          type="expense"
        />
        <app-panel-card
          label="SALDO MENSAL"
          [value]="
            (data.stats.monthlyBalance | currency: 'BRL' : 'symbol' : '1.2-2' : 'pt-BR') ||
            'R$ 0,00'
          "
          type="balance"
        />
      ...
      <section
        *ngIf="data && !isLoading"
        class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-[1fr_400px] gap-4 min-w-0"
      >
        <article
          class="min-w-0 p-4 sm:p-6 rounded-xl border border-stone-200 bg-white shadow-sm md:col-span-2 lg:col-span-1"
        >
          <header class="flex flex-col sm:flex-row sm:justify-between sm:items-center mb-6 gap-3">
            ...
            <div class="flex items-center gap-3 flex-wrap">
              <div class="flex items-center gap-1.5">
                ...
                <span
                  class="text-xs sm:text-[10px] font-bold tracking-wide uppercase text-stone-500"
                  >RECEITA</span
                >
              </div>
              <div class="flex items-center gap-1.5">
          ...
          <div style="position:relative; height:280px; min-height:200px; min-width:0;">
            <canvas #barCanvas role="img" aria-label="Gráfico de barras de receita e despesa mensal"
              >Dados mensais de receita e despesa.</canvas
            >
          </div>
        </article>

        <article
          class="min-w-0 p-4 sm:p-6 rounded-xl border border-stone-200 bg-white shadow-sm md:col-span-2 lg:col-span-1 lg:row-span-1"
        >
          <h2 class="font-serif text-lg font-bold text-stone-800 m-0 mb-6">Receita por Serviço</h2>
          <div
            class="flex flex-col sm:flex-row lg:flex-col items-center justify-center gap-6 sm:gap-8 min-w-0"
          >
            <div
              style="position:relative; width:180px; height:180px; flex-shrink:0; min-width:180px;"
            >
              <canvas #pieCanvas role="img" aria-label="Gráfico de rosca com receita por serviço"
                >Receita por serviço.</canvas
              >
            </div>
...
export class PanelPageComponent implements OnInit, OnDestroy {
  ...
  protected transactionModalOpen = false;
  ...
  protected saveTransaction(payload: FinancialTransactionCreatePayload): void {
    ...
    this.panelService.createTransaction(payload).subscribe({
      ...
      error: (err) => {
        this.savingTransaction = false;
        this.transactionError = err.message ?? 'Não foi possível salvar a transação.';
        this.cdr.markForCheck();
      },
    });
  }
  ...
  protected get revenueBadge(): string | undefined {
    return this.percentChangeLabel(this.previousMonth?.receita, this.currentMonth?.receita);
  }

  protected get expenseBadge(): string | undefined {
    return this.percentChangeLabel(this.previousMonth?.despesa, this.currentMonth?.despesa);
  }

  private get currentMonth(): MonthlyTrend | undefined {
    return this.data?.monthlyTrend.at(-1);
  }

  private get previousMonth(): MonthlyTrend | undefined {
    const trend = this.data?.monthlyTrend ?? [];
    return trend.length > 1 ? trend[trend.length - 2] : undefined;
  }

  private percentChangeLabel(
    previous: number | undefined,
    current: number | undefined,
  ): string | undefined {
    if (previous == null || current == null || previous === 0) return undefined;
  ...
  private tryBuildCharts(): void {
    ...
    let trend = this.data.monthlyTrend.filter((m) => m.receita > 0 || m.despesa > 0);
    if (trend.length === 0) {
      trend = this.data.monthlyTrend;
    }
