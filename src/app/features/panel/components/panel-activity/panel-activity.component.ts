  ...
  template: `
    <div class="bg-white rounded-xl border border-stone-200 shadow-sm overflow-hidden">
      ...
      <header class="flex flex-col gap-3 p-4 sm:p-5 pb-4 border-b border-stone-200">
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          ...
          <div class="flex items-center gap-2 flex-wrap">
            <!-- BUSCA -->
            <label class="flex items-center gap-2 px-3 py-1.5 border border-stone-200 rounded-lg bg-stone-50 focus-within:ring-2 focus-within:ring-emerald-500/30 focus-within:border-emerald-500/40 transition flex-1 sm:flex-none">
              <svg width="11" height="11" viewBox="0 0 11 11" fill="none" class="text-stone-400 shrink-0">
                <path d="M9.683 10.5 6.008 6.825c-.291.234-.625.418-1.005.554C4.623 7.515 4.22 7.583 3.792 7.583 2.732 7.583 1.835 7.216 1.101 6.482.367 5.748 0 4.851 0 3.792 0 2.732.367 1.835 1.101 1.101 1.835.367 2.732 0 3.792 0c1.059 0 1.956.367 2.69 1.101C7.216 1.835 7.583 2.732 7.583 3.792c0 .427-.068.83-.204 1.21-.136.379-.32.713-.554 1.005L10.5 9.683 9.683 10.5ZM3.792 6.417c.729 0 1.349-.256 1.859-.766.51-.51.766-1.13.766-1.859 0-.729-.256-1.35-.766-1.859C5.141 1.423 4.521 1.167 3.792 1.167c-.729 0-1.35.256-1.859.766-.51.51-.766 1.13-.766 1.859 0 .729.256 1.349.766 1.859.51.51 1.13.766 1.859.766Z" fill="currentColor"/>
              </svg>
              <input
                type="text"
                [(ngModel)]="searchTerm"
      ...
      <div class="block md:hidden">
        <div
          *ngFor="let a of filteredActivities; trackBy: trackById"
          class="flex items-center gap-3 px-3 sm:px-4 py-3 sm:py-3.5 border-t border-stone-100"
        >
          <div
            class="w-9 sm:w-10 h-9 sm:h-10 rounded-full flex items-center justify-center shrink-0"
            [ngClass]="a.category === 'RECEITA' ? 'bg-emerald-100 text-emerald-600' : 'bg-red-100 text-red-600'"
          >
            <svg *ngIf="a.category === 'RECEITA'" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="M12 5v14M5 12l7 7 7-7"/></svg>
            <svg *ngIf="a.category === 'DESPESA'" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="M12 19V5M5 12l7-7 7 7"/></svg>
          </div>
          ...
          <div class="text-right shrink-0">
            ...
            <p class="text-[8px] sm:text-[9px] font-bold uppercase m-0 mt-1" [style.color]="getStatusColor(a.status)">{{ a.status }}</p>
          </div>
        </div>
        <div *ngIf="filteredActivities.length === 0" class="px-4 py-8 text-center text-sm text-stone-400 border-t border-stone-100">
          Nenhuma transação encontrada.
        </div>
      </div>
